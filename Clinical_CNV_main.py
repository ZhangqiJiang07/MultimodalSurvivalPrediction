import sys
sys.path.append('different_models/Clinical_CNV')
import numpy as np
import pandas as pd
import torch
...
from Clinical_CNV_network import *
from TwoModal_utils import *
from utils import *

#For Clinical and CNV
...
def main():
	...
	mydataset = MyDataset(PATH_CNV, PATH_CLIN)
	...
	for j in range(K):
		...
		#Create New Net
		net = AtteNet(cnv_lenth=1739, embedding_sizes=embedding_size, n_cont=1, n_model=2, m_lenth=128)
		net.to(device)
		...
		for epoch in range(EPOCH):
			...
			#Training set
			net.train()
			for train_step, (cnv_batch, clin_cat_batch, clin_cont_batch, target_batch) in enumerate(train_loader):
				out, v = net(cnv=cnv_batch.to(device), clin_cat=clin_cat_batch.to(device), clin_cont=clin_cont_batch.to(device))
			...
			#Test set
			net.eval()
			for val_step, (cnv_batch, clin_cat_batch, clin_cont_batch, target_batch) in enumerate(validation_loader):
				out, _ = net(cnv=cnv_batch.to(device), clin_cat=clin_cat_batch.to(device), clin_cont=clin_cont_batch.to(device))
