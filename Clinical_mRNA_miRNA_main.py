import sys
sys.path.append('different_models/Clinical_miRNA_mRNA')
import numpy as np
import pandas as pd
import torch
...
from Clinical_mRNA_miRNA_network import *
from ThreeModal_utils import *
from utils import *

#For Clinical, mRNA and miRNA
...
def main():
	...
	mydataset = MyDataset(PATH_MRNA, PATH_MIRNA, PATH_CLIN)
	...
	for j in range(K):
		...
		#Create New Net
		net = AtteNet(mrna_lenth=1176, mirna_lenth=743, embedding_sizes=embedding_size, n_cont=1, n_model=3, m_lenth=128)
		net.to(device)
		...
		for epoch in range(EPOCH):
			...
			#Training set
			net.train()
			for train_step, (mrna_batch, mirna_batch, clin_cat_batch, clin_cont_batch, target_batch) in enumerate(train_loader):
				out, v = net(mrna=mrna_batch.to(device), mirna=mirna_batch.to(device), clin_cat=clin_cat_batch.to(device), clin_cont=clin_cont_batch.to(device))
			...
			'''Test set'''
			net.eval()
			for val_step, (mrna_batch, mirna_batch, clin_cat_batch, clin_cont_batch, target_batch) in enumerate(validation_loader):
				out, _ = net(mrna=mrna_batch.to(device), mirna=mirna_batch.to(device), clin_cat=clin_cat_batch.to(device), clin_cont=clin_cont_batch.to(device))
	...
	print('经过5-Fold Cross Validation检验之后得到的平均结果是：')
	print(mean_value)
