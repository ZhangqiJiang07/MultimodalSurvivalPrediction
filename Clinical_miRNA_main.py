import sys
sys.path.append('different_models/Clinical_mirna')
import numpy as np
import pandas as pd
import torch
...
from Clinical_miRNA_network import *
from TwoModal_utils import *
from utils import *

#For Clinical and miRNA
...
def main():
	...
	mydataset = MyDataset(PATH_MIRNA, PATH_CLIN)
	...
	for j in range(K):
		...
		#Create New Net
		net = AtteNet(mirna_lenth=743, embedding_sizes=embedding_size, n_cont=1, n_model=2, m_lenth=128)
		net.to(device)
		...
		for epoch in range(EPOCH):
			...
			#Training set
			net.train()
			for train_step, (mirna_batch, clin_cat_batch, clin_cont_batch, target_batch) in enumerate(train_loader):
				out, v = net(mirna=mirna_batch.to(device), clin_cat=clin_cat_batch.to(device), clin_cont=clin_cont_batch.to(device))
			...
			'''Test set'''
			net.eval()
			for val_step, (mirna_batch, clin_cat_batch, clin_cont_batch, target_batch) in enumerate(validation_loader):
				out, _ = net(mirna=mirna_batch.to(device), clin_cat=clin_cat_batch.to(device), clin_cont=clin_cont_batch.to(device))
