...
for cancer_type in tested_cancer_type:
	...
	test_c_index_arr = []
	for train_val, test in trainVal_test_kfold:
		test_sampler = cancer_sample_true_index[test]
		train_val_df_X, train_val_df_y = prepro_clin_data_X.loc[cancer_sample_true_index[train_val], :], prepro_clin_data_y.loc[cancer_sample_true_index[train_val], :]
		train_X, val_X, _, _ = train_test_split(train_val_df_X, train_val_df_y, test_size=0.25, random_state=SEED, stratify=train_val_df_y[[6]])
...
with open('figure3_single_cancer_clinicalmRNA.txt', 'w') as file:
	for k in result_dic:
		file.write(f'{k}************\n')
