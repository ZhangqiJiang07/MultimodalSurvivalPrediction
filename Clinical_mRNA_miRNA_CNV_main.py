import sys
sys.path.append('different_models/Clinical_miRNA_mRNA_CNV')
import numpy as np
import pandas as pd
import torch
...
from Clinical_mRNA_miRNA_CNV_network import *
from FourModal_utils import *
from utils import *

#For Clinical, mRNA, miRNA and CNV
PATH_CLIN = 'data/PC_Clinical.csv'
PATH_MRNA = 'data/PC_mRNA_0_1.csv'
PATH_MIRNA = 'data/PC_miRNA_0_1.csv'
...
def main():
	...
	mydataset = MyDataset(PATH_MRNA, PATH_MIRNA, PATH_CNV, PATH_CLIN)
	...
	for j in range(K):
		...
		#Create New Net
		net = AtteNet(mrna_lenth=1176, mirna_lenth=743, cnv_lenth=1739,  embedding_sizes=embedding_size, n_cont=1, n_model=4, m_lenth=128)
		net.to(device)
		...
		for epoch in range(EPOCH):
			...
			#Training set
			net.train()
			for train_step, (mrna_batch, mirna_batch, cnv_batch, clin_cat_batch, clin_cont_batch, target_batch) in enumerate(train_loader):
				out, v = net(mrna=mrna_batch.to(device), mirna=mirna_batch.to(device), cnv=cnv_batch.to(device), clin_cat=clin_cat_batch.to(device), clin_cont=clin_cont_batch.to(device))
			...
			'''Test set'''
			net.eval()
			for val_step, (mrna_batch, mirna_batch, cnv_batch, clin_cat_batch, clin_cont_batch, target_batch) in enumerate(validation_loader):
				out, _ = net(mrna=mrna_batch.to(device), mirna=mirna_batch.to(device), cnv=cnv_batch.to(device), clin_cat=clin_cat_batch.to(device), clin_cont=clin_cont_batch.to(device))
