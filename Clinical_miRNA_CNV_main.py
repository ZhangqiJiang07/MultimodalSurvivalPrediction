import sys
sys.path.append('different_models/Clinical_miRNA_CNV')
import numpy as np
import pandas as pd
import torch
...
from Clinical_miRNA_CNV_network import *
from ThreeModal_utils import *
from utils import *

#For Clinica, miRNA and CNV
...
def main():
	...
	mydataset = MyDataset(PATH_MIRNA, PATH_CNV, PATH_CLIN)
	...
	for j in range(K):
		...
		#Create New Net
		net = AtteNet(mirna_lenth=743, cnv_lenth=1739, embedding_sizes=embedding_size, n_cont=1, n_model=3, m_lenth=128)
		net.to(device)
		...
		for epoch in range(EPOCH):
			...
			#Training set
			net.train()
			for train_step, (mirna_batch, cnv_batch, clin_cat_batch, clin_cont_batch, target_batch) in enumerate(train_loader):
				out, v = net(mirna=mirna_batch.to(device), cnv=cnv_batch.to(device), clin_cat=clin_cat_batch.to(device), clin_cont=clin_cont_batch.to(device))
			...
			'''Test set'''
			net.eval()
			for val_step, (mirna_batch, cnv_batch, clin_cat_batch, clin_cont_batch, target_batch) in enumerate(validation_loader):
				out, _ = net(mirna=mirna_batch.to(device), cnv=cnv_batch.to(device), clin_cat=clin_cat_batch.to(device), clin_cont=clin_cont_batch.to(device))
