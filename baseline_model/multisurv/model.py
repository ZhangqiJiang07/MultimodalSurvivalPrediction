...
class _BaseModelWithDataLoader:
	def __init__(self, modalities, m_length, dataloaders, device=None):
	...
	def _instantiate_model(self, move2device=True):
		...
		self.model = Net(self.data_modalities, self.m_length, self.device)
...
class Model(_BaseModelWithDataLoader):
	def __init__(self, modalities, m_length, dataloaders, device=None):
		super().__init__(modalities, m_length, dataloaders, device)
