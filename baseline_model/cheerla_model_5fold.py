...
for modalities in modalities_list:
	...
	for train_val_cross, test_cross in trainVal_test_kfold:
		...
		cheerla_model = Model(
			...
			fusion_method='average', #attention
			trade_off=0.3,
			mode='total', # only_cox
			device=device)

		run_tag = utils.compose_run_tag(
			model=cheerla_model, lr=lr, dataloaders=dataloaders,
		...
		# map cancer type to crrospond embedded index
			...
			for data, data_label in dataloaders_cancer['test']:
				out, event, time = cheerla_model.predict(data, data_label)
	...
	with open(f'figure4_cheerlamodel_{dir_name}.txt', 'w') as file:
		for k in contain:
