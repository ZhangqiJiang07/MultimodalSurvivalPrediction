...
from sub_models.sumFusion import AverageFusion
...
class Net(nn.Module):
	def __init__(self, modalities, m_length, fusion_method='attention', device=None,
		...
		if 'clinical' in self.data_modalities:
			self.clinical_submodel = ClinicalEmbeddingNet(m_length=self.m_length, device=self.device)
			self.submodel_pipeline['clinical'] = self.clinical_submodel

		# mRNA ---------------------------------------------------#
		if 'mRNA' in self.data_modalities:
			self.mRNA_submodel = MrnaNet(mrna_length=self.dim['mRNA'], m_length=self.m_length, device=self.device)
			self.submodel_pipeline['mRNA'] = self.mRNA_submodel

		# miRNA --------------------------------------------------#
		if 'miRNA' in self.data_modalities:
			self.miRNA_submodel = MirnaNet(mirna_length=self.dim['miRNA'], m_length=self.m_length, device=self.device)
			self.submodel_pipeline['miRNA'] = self.miRNA_submodel

		# CNV ----------------------------------------------------#
		if 'CNV' in self.data_modalities:
			self.CNV_submodel = CnvNet(cnv_length=self.dim['CNV'], m_length=self.m_length, device=self.device)
		...
		if len(self.data_modalities) > 1:
			...
			elif fusion_method == 'fixedAttention':
				self.fusion = FixedAttention(m_length=self.m_length, modalities=self.data_modalities, device=self.device)
			elif fusion_method == 'average':
				self.fusion = AverageFusion(m_length=self.m_length, modalities=self.data_modalities, device=self.device)
