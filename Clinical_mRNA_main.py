import sys
sys.path.append('different_models/Clinical_mRNA')
import numpy as np
import pandas as pd
import torch
...
from Clinical_mRNA_network import *
from TwoModal_utils import *
from utils import *

#For Clinical and mRNA
...
def main():
	...
	mydataset = MyDataset(PATH_MRNA, PATH_CLIN)
	...
	for j in range(K):
		...
		#Create New Net
		net = AtteNet(mrna_lenth=1176, embedding_sizes=embedding_size, n_cont=1, n_model=2, m_lenth=128)
		net.to(device)
		...
		for epoch in range(EPOCH):
			...
			#Training set
			net.train()
			for train_step, (mrna_batch, clin_cat_batch, clin_cont_batch, target_batch) in enumerate(train_loader):
				out, v = net(mrna=mrna_batch.to(device), clin_cat=clin_cat_batch.to(device), clin_cont=clin_cont_batch.to(device))
			...
			'''Test set'''
			net.eval()
			for val_step, (mrna_batch, clin_cat_batch, clin_cont_batch, target_batch) in enumerate(validation_loader):
				out, _ = net(mrna=mrna_batch.to(device), clin_cat=clin_cat_batch.to(device), clin_cont=clin_cont_batch.to(device))
