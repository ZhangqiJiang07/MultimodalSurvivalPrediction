...
class AtteNet(nn.Module):
    ...
    def __init__(self, mirna_lenth, embedding_sizes, n_cont, n_model, m_lenth):
        ...
        #Dropout
        self.dropout_layer1 = nn.Dropout(p=0.7)

    def forward(self, mirna, clin_cat, clin_cont):
        mirna = torch.relu(self.bn1(self.mirna_hidden1(mirna)))
        mirna = torch.relu(self.bn2(self.mirna_hidden2(mirna)))
        mirna = torch.relu(self.bn3(self.mirna_hidden3(mirna)))
        mirna = torch.relu(self.bn4(self.mirna_hidden4(mirna)))

        clin = self.clin_embedding_net(clin_cat, clin_cont)
        samples = mirna.shape[0]

        #Fusion
        v = atten_fusion(mirna, clin)
