...
class AtteNet(nn.Module):
    ...
    def __init__(self, cnv_lenth, embedding_sizes, n_cont, n_model, m_lenth):
    ...
    def forward(self, cnv, clin_cat, clin_cont):
        cnv = torch.relu(self.bn1(self.cnv_hidden1(cnv)))
        ...
        cnv = torch.relu(self.bn5(self.cnv_hidden5(cnv)))

        clin = self.clin_embedding_net(clin_cat, clin_cont)
        samples = cnv.shape[0]

        v = atten_fusion(cnv, clin)
