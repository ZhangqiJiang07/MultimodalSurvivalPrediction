...
def atten_fusion4(info1, info2, info3, info4):
    ...
    d = info4.view(sample_num, 1, info_lenth, 1)
    fused_info = torch.cat([a, b, c, d], dim=1)
    return fused_info
...
class AtteNet(nn.Module):
    ...
    def __init__(self, mrna_lenth, mirna_lenth, cnv_lenth, embedding_sizes, n_cont, n_model, m_lenth):
        super(AtteNet, self).__init__()
        self.mrna_lenth = mrna_lenth
        self.mirna_lenth = mirna_lenth
        ...
        # mirna Sub-Model
        self.mirna_hidden1 = nn.Linear(mirna_lenth, 600)
        self.mirna_hidden2 = nn.Linear(600, 400)
        self.mirna_hidden3 = nn.Linear(400, 200)
        self.mirna_hidden4 = nn.Linear(200, m_lenth)
        # cnv Sub-Model
        ...
        self.mirna_bn1 = nn.BatchNorm1d(600)
        self.mirna_bn2 = nn.BatchNorm1d(400)
        self.mirna_bn3 = nn.BatchNorm1d(200)
        self.mirna_bn4 = nn.BatchNorm1d(128)
        ...
        self.dropout_layer3 = nn.Dropout(p=0.3)

    def forward(self, mrna, mirna, cnv, clin_cat, clin_cont):
        ...
        mirna = torch.relu(self.mirna_bn1(self.mirna_hidden1(mirna)))
        mirna = torch.relu(self.mirna_bn2(self.mirna_hidden2(mirna)))
        mirna = torch.relu(self.mirna_bn3(self.mirna_hidden3(mirna)))
        mirna = torch.relu(self.mirna_bn4(self.mirna_hidden4(mirna)))
        ...
        v = atten_fusion4(mrna, mirna, cnv, clin)
