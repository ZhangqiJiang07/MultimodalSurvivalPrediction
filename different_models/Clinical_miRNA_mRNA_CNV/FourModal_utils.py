...
class MyDataset(torch.utils.data.Dataset):
    def __init__(self, path_data1, path_data2, path_data3, path_clin):
        ...
        data2 = pd.read_csv(path_data2, header=None)
        data2.drop(index=out_idx + g, inplace=True)
        # data2.reset_index(drop=True)
        self.data2 = data2.values.tolist()

        data3 = pd.read_csv(path_data3, header=None)
        data3.drop(index=out_idx + g, inplace=True)
        # data3.reset_index(drop=True)
        self.data3 = data3.values.tolist()
    ...
    def __getitem__(self, index):
        ...
        data_m2 = np.array(self.data2[index])
        data_m2 = torch.from_numpy(data_m2)
        data_m2 = data_m2.type(torch.FloatTensor)

        data_m3 = np.array(self.data3[index])
        data_m3 = torch.from_numpy(data_m3)
        data_m3 = data_m3.type(torch.FloatTensor)

        target_y = np.array(self.target[index], dtype='int')
        target_y = torch.from_numpy(target_y)
        target_y = target_y.type(torch.LongTensor)

        return data_m1, data_m2, data_m3, clin_cat, clin_cont, target_y
